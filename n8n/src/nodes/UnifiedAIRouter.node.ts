...
import { AIRouter, Provider, Message } from '../AIRouter';

export class UnifiedAIRouter implements INodeType {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    const providersInput = this.getNodeParameter('providers', 0) as any;
    const providerList: Provider[] = providersInput.provider || [];

    const messagesInput = this.getNodeParameter('messages', 0) as any;
    const messageList: Message[] = messagesInput.message || [];

    const options = this.getNodeParameter('options', 0) as any;

    const aiRouter = new AIRouter(providerList);
